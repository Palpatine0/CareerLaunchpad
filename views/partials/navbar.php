<!-- Navigation Bar -->
<header class="text-white p-4" style="background-color: #ffffff;">
    <div class="container mx-auto flex justify-between items-center">
        <h1 class="text-3xl font-semibold">
            <img href="/public/" class="vert" src="https://www.asu.edu/modules/webspark/asu_brand/node_modules/@asu/component-header/dist/assets/img/arizona-state-university-logo-vertical.png" alt="Arizona State University" title="ASU home page" width="80" height="234" decoding="async" fetchpriority="high">
            <!--<a href="index.html" style="color:#000;">Arizona State University</a>-->
        </h1>
        <nav class="space-x-4">
            <a href="login.html" class="text-white hover:underline">Login</a>
            <a href="register.html" class="text-white hover:underline">Register</a>
            <a href="/public/listings/publish" class="bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded hover:shadow-md transition duration-300">
                <i class="fa fa-edit"></i>
                Post Internship
            </a>
        </nav>
    </div>
</header>